...
*/
export const suggestFees = async (provider: JsonRpcProvider, sampleMin = 0.1, sampleMax = 0.3, maxTimeFactor = 15, extraTipRatio = 0.5, fallbackTip = 5e9) => {
    ...
    const tip = await suggestTip(feeHistory.oldestBlock, feeHistory.gasUsedRatio, fallbackTip, provider);
    const result = [];
    let maxBaseFee = 0;
...
const suggestTip = async (firstBlock: number, gasUsedRatio: string | any[], fallbackTip: number, provider: JsonRpcProvider) => {
    ...
    while (needBlocks > 0 && ptr >= 0) {
        ...
        if (blockCount > 0) {
            ...
            const feeHistory = await provider.send('eth_feeHistory', [blockCount, BigNumber.from(firstBlock + ptr).toHexString(), [10]]);
...
const maxBlockCount = (gasUsedRatio: string | any[], ptr: number, needBlocks: number) => {
    let blockCount = 0;
    while (needBlocks > 0 && ptr >= 0) {
...
const suggestBaseFee = (baseFee: string | any[], order: string | any[], timeFactor: number, sampleMin: number, sampleMax: number) => {
    if (timeFactor < 1e-6) {
...
const samplingCurve = (sumWeight: number, sampleMin: number, sampleMax: number) => {
